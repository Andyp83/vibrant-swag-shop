ALTER TABLE public.catalog_products
  ...
  ADD COLUMN IF NOT EXISTS supplier_meta jsonb NOT NULL DEFAULT '{}'::jsonb;

CREATE INDEX IF NOT EXISTS catalog_products_publish_status_idx
  ON public.catalog_products (publish_status);

-- New overload-safe function name so the currently deployed app keeps using the
-- existing search_product_families without ambiguity.
CREATE OR REPLACE FUNCTION public.search_product_families_v2(
  p_category_id uuid DEFAULT NULL::uuid,
  p_subcategory_id uuid DEFAULT NULL::uuid,
  p_decoration text DEFAULT NULL::text,
  p_colour_terms text[] DEFAULT '{}'::text[],
  p_colour_mode text DEFAULT 'any'::text,
...
GRANT EXECUTE ON FUNCTION public.search_product_families_v2(uuid, uuid, text, text[], text, boolean, integer, text, integer, integer, uuid[], boolean) TO anon, authenticated, service_role;
